<template>
  <div class="detail">
    ...
    <el-card>
      <el-button type="danger" @click="gettinyinfo">确认</el-button>
    </el-card>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    gettinyinfo() {
      ...
      // console.log(this.xyNews.hot)
      newsApi.updateList(obj).then(res => {
        // console.log(res)
    ...
    getinfo(i) {
      newsApi.detailnews({ id: i }).then(res => {
        // console.log(this.newcontent)
        this.xyNews = res.data;
        this.xyNews.content1 = res.data.content1;
      });
    }
  },
  mounted() {
    this.xyNews.id = this.$route.params.id;
    this.getinfo(this.xyNews.id);
  },
  created() {
